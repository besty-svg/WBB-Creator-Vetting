...
const BRAND_PRESETS = [
  ...
  {
    name: "Greedy Games",
    description: "Greedy Games helps mobile game brands grow through creator-led performance campaigns.",
    senderName: "Dun",
    senderRole: "Influencer Specialist at Work Beyond Borders",
    usage: "90 days"
  }
...
function defaultBrand() {
  return cloneBrandPreset("Printify");
}

function cloneBrandPreset(name) {
  const preset = BRAND_PRESETS.find((brand) => brand.name.toLowerCase() === String(name).toLowerCase()) || BRAND_PRESETS[0];
  return { ...preset };
}

function brandIndexByName(name) {
  const key = String(name || "").toLowerCase();
  return brands.findIndex((brand) => String(brand.name || "").toLowerCase() === key);
}

function ensureBrandExists(name) {
  let index = brandIndexByName(name);
  if (index !== -1) return index;
  brands.push(cloneBrandPreset(name));
  return brands.length - 1;
}

function ensurePresetBrands() {
  BRAND_PRESETS.forEach((brand) => ensureBrandExists(brand.name));
...
function refreshBrandNav() {
  const active = activeBrand();
  $$(".brand-quick").forEach((button) => {
    const isActive = active && String(active.name).toLowerCase() === String(button.dataset.brandName || "").toLowerCase();
...
function selectBrandByName(name) {
  selectedBrandIndex = ensureBrandExists(name);
  saveBrands();
  refreshBrandSelect();
  updateMessage();
}
...
function bindEvents() {
  ...
  $$(".brand-quick").forEach((button) => {
    button.addEventListener("click", () => selectBrandByName(button.dataset.brandName));
  });
